...
{
       seteuid(geteuid());
      set_name(HIC"老婆爱我" NOR, ({"loveme","love"}));
...
int do_die (string str)
{
  ...
  tell_object(ob,"");  
  ob->die();
//  message_vision( sprintf( HIY "只见$N用手对着天空一指，大叫：“宝剑回鞘”, 一把巨剑从天空中回到了$N身后的剑鞘里。\n" NOR),
//         this_player());
//  write("Ok.\n");
  return 1;
}
  ...
  int do_gan (string str)
{
  ...
  tell_object(ob,"「兔宝宝」告诉你：实在不好意思，请你下线去吧...\n");
  destruct(ob);
...
int do_halt (string str)
{
  ...
  tell_room(environment(ob),HIY "天空中忽然出现了月亮的身影。\n她温柔的对"+                    
