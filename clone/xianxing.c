...
int filter_user(object ob)
{
        ...
        if (ob->query("env/invisible"))
        {
                ...
                CHANNEL_D->do_channel(this_object(), "rumor", "听说" HIW+ ob->query("name") +HIM
                                      "被炸得现了形。");
                return 1;
        }
        ...
        if (! undefinedp(ob->query_temp("apply/name")))
        {
                ...
                msg = "听说" HIW+ ob->query("name") +HIM "假扮" HIY+ ob->name() +HIM
                      "被"HIR"现形炸弹"HIM"炸得“嗷嗷”直叫，现出了原形，浑身上下都是硝烟。";
