...
int check_result(object me, object ob)
{
	...
	if (((int)me->query("kee")*100/(1+my_max_kee)) <= 50 )
	{
		...
		if (me->query_temp("zhangmen/kill") && ob->query_temp("shushan_zm_applied"))
		{
...
void master_announce (object me, object who, object ob)
{
	...
	ob->delete_temp("shushan_zm_applied");
	me->set("new_player",1);
...
int rank_me()
{
	...
	if (me->query("title") == HIC"蜀山剑派掌门剑子"NOR || 
	    me->query("title") == HIC"蜀山剑派掌门剑姬"NOR)
		return notify_fail("你已为掌门亲传大弟子，剑道修为已臻上乘。\n");
	...
	if (me->query("family/family_name") == "蜀山剑派")
	{
		if (exp <= 20000) {
			...
			me->set("title", "蜀山剑派守剑弟子");
		} else if (exp <= 100000) {
			me->set("title", "蜀山剑派执剑弟子");
		} else if (exp <= 200000) {
			me->set("title", "蜀山剑派御剑使者");
		} else if (exp <= 500000) {
			me->set("title", "蜀山剑派飞剑长老");
		} else if (exp <= 1000000) {
			me->set("title", "蜀山剑派青冥客");
		} else if (exp <= 2000000) {
			me->set("title", "蜀山剑派问道真人");
		} else {
			// 根据性别设置掌门大弟子称号
			if (me->query("gender") == "女性")
				me->set("title", HIC"蜀山剑派素心剑姬"NOR);
			else
				me->set("title", HIC"蜀山剑派清虚剑子"NOR);
		}

		tell_room(environment(me),
			HIW + ob->query("name") + "负手而立，肃然道：\n"
			+ me->query("name") + "剑心通明，心怀苍生，已得我蜀山剑意真传！\n"
			"今授其「" + me->query("title") + "」之号，望你持剑明道，济世安人，不负剑宗清誉！\n"NOR,
			({ me, me }));
	}
	return 1;
}
