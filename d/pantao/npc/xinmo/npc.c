...
void copy_status(object me,object ob)
{
        ...
        me->set_name(ob->query("name")+MAG"心魔"NOR,({ob->query("id")+" npc","npc"}));
...
void die()
{
      ...
      switch(random(2))
        ...
        sprintf("挑衅%s的"NOR"「"YEL+"%s」"NOR+HIR"被"NOR+HIM"%s"NOR+HIR"给灭了。"NOR,query("target"),query("name"), ob->name() ));
              break;
              case 2:
             CHANNEL_D->do_channel( this_object(), "rumor",
        sprintf("挑衅%s的"NOR"「"YEL+"%s」"NOR+HIR"被"NOR+HIM"%s"NOR+HIR"送上西天了。"NOR,query("target"),query("name"), ob->name() ));
              break;
              case 1:
        message("system",HIC"〖"NOR+CYN"门派"NOR+HIC"〗"NOR+CYN+ob->query("family/family_name")+"掌门(Zhangmen)："+"本门爱徒"NOR+HIM+ob->name()+NOR+CYN"把挑衅"+query("target")+"的"NOR+query("name")+CYN"给灭了，成功保护了本门派的安全，受到本门派的嘉奖！\n"NOR,users());
            break;
           }
