...
//	command("chat* "+name()+"：各位老爷太太少爷小姐，今日" + who->query("name") +"在喜福会大开酒席，欢迎各位前来捧场！");
	command("say 您要开始(start)，我便开席。您要上菜(serve)，我就上菜，等您吃饱了，玩腻了，咱就结束(finish)。");
message("shout", HIY"〖"HIR"喜宴"HIY"〗："HIR+this_player()->name()+HIY"在"RED"「"YEL"喜福会"RED"」"HIY"设宴款待各路好汉,欢迎各位前来捧场。\n"NOR,users()); 	me->set_temp("ready_to_party", 1);
}
int do_start()
{	
	object who = this_player();
