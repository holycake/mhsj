...
void die()
{
    string skill;
       ...
       if (owner_ob->query_temp("mieyao/level1")==8 && ratio>10) {
 
          skill=give_reward(owner_ob);
         str+= "and rewarded 1 lvl of "+skill+".\n";
    message("shout",HIG"〖"HIR"大唐降妖"HIY"〗"HIG"袁天罡(Yuan tiangang):" HIR+ owner_ob->query("name")+HIM"协助天庭斩妖除魔有功,玉皇大帝赐"HIY+ to_chinese(skill)+HIM"一级！\n"NOR,users()); 
                    MONITOR_D->report_system_object_msg(owner_ob, str);
...
*/
            ...
            message_vision("$N"HIY"口吐"HIR"鲜血"HIY"，眼皮狠狠的翻了一下，"HIR"死了～～\n"NOR,this_object());
        }
