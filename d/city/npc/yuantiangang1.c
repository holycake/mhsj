...
{	string where;
	...
	if(env)
	  here="在"+MISC_D->find_place(env);
	message_vision("$N将手中桃木剑向四方一划，对$n说道：\n",
	  this_object(),me);
	message_vision("　　近有"+where+"出没，为害不小，"
		+"请速去收服！\n"
		,this_object(),me);
	announce (this_object(),me->query("name")+"前去斩杀"+where+"！\n");
	me->set("mieyao/time_start1", time());
	me->set("mieyao/name1",ghost->query("name"));
	me->set("mieyao/guaiid",ghost->query("id"));
	me->set("mieyao/where",here);
	return 1;
}
