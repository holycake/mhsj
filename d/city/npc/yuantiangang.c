...
//	dx=(me->query("daoxing")+me->query("combat_exp"))/2;
	...
	if(dx>2700000) {
	  message_vision(HIY"$N对$n一拱手：这位"HIW+
	    RANK_D->query_respect(me)+HIY
	    "请速去天廷协助灭妖！\n"NOR,this_object(),me);
	  return 1;
	}
...
//                    time()<t+900) {
	    ...
	    if(t<=time() &&
                    time()<t+(dx>20000?0:0)) {
//                    time()<t+(dx>20000?120:0)) {
		message_vision(HIY"$N将手中"NOR YEL"桃木剑"HIY"缓缓放下，说"+
			"："HIR"多谢"+RANK_D->query_respect(me)
			+HIR",妖魔已经除尽了。\n"NOR,
			this_object(), me);
		return 1;
	    }
...
{	string where;
	...
	message_vision("$N将手中桃木剑向四方一划，对$n说道：\n",
	  this_object(),me);
	message_vision("　　近有"NOR+where+NOR"鱼肉百姓，为害不小，"
		+"请速去将其收服！\n"NOR
		,this_object(),me);
