	...
	int max_kee, max_sen, ratio, exp_r, pot_r;
        ...
        if (owner_ob->query_temp("mieyao/level")==9)
                {
                        ...
                        tell_object(owner_ob, HIY"你得到了"HIR+COMBAT_D->chinese_daoxing(dx_r)
        +"道行!"NOR);
		...
		if (owner_ob->query_temp("mieyao/level")==9 && ratio>50) {
         ...
         str+= "并且被奖励了一级 "+to_chinese(skill)+".\n";
   		if (skill != "none"){
    message("shout",HIY"〖"HIW"斩妖除魔"HIY"〗"HIY"李靖(Li jing):" HIW+ owner_ob->query("name")+HIM"协助天庭斩妖除魔有功,玉皇大帝赐"HIY+ to_chinese(skill)+HIM"一级！\n"NOR,users()); CHANNEL_D->do_channel(me,"rumor",msg);
    }
