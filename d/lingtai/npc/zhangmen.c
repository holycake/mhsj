...
string zname(object ob)
{
	if( (string)ob->query("gender") == "女性" ) 
		return HIG"万妙无方神姬"NOR;
	else return HIG"万妙无方天尊"NOR;
}
...
int rank_me()
{
	...
	exp=(int)me->query("combat_exp");
        if( me->query("title") == HIG"方寸山三星洞万妙无方天尊"NOR
          ||me->query("title") == HIG"方寸山三星洞万妙无方神姬"NOR)
	...
	if( me->query("family/family_name") == "方寸山三星洞")
	{
		...
		tell_room(environment(me), HIR""+ob->query("name")+"肃然宣道："+me->query("name")+"品端志坚，言行有度，道心不移，修行有成，实为门中栋梁。\n今以掌教之名，特授「"+me->query("title")+"」之号，望尔严持清修，广弘道统，不辱门风。\n"NOR, ({me, me}));
	}
        }
        else
        {
	if( me->query("family/family_name") == "方寸山三星洞")
	{
		...
		tell_room(environment(me), HIR""+ob->query("name")+"肃然宣道："+me->query("name")+"心慧性坚，言止有章，端谨有德，修业精勤，堪为门中楷模。\n今以掌教之名，特授「"+me->query("title")+"」之号，望尔谨遵门规，持守正道，不负师门厚望。\n"NOR, ({me, me}));
	}
        }
	return 1;
}
