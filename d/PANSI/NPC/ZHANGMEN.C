...
int convert_identity (object me, object ob)
{
	...
	me->set("short",HIM+me->query("name")+NOR+"("+HIY+capitalize(me->query("id"))+NOR+")");
...
string zname(object ob)
{
	/*if( (string)ob->query("gender") == "女性" ) 
		return MAG"盘丝洞续缘玉女"NOR;
	else return MAG"盘丝洞忘忧公子"NOR;*/
	retrun MAG"盘丝洞紫青妖皇"NOR;
}
...
int rank_me()
{
	...
	if (me->query("title") == HIR"盘丝洞紫青妖皇"NOR)
		return notify_fail("你已经是盘丝洞至尊头衔，再无更高之位可授。\n");

	if (me->query("family/family_name") == "花果山盘丝洞")
	{
		if (exp <= 20000) {
			me->set("title", "盘丝洞小蛛妖");
		} else if (exp <= 50000) {
			me->set("title", "盘丝洞引丝侍姬");
		} else if (exp <= 100000) {
			me->set("title", "盘丝洞花影妖婢");
		} else if (exp <= 200000) {
			me->set("title", "盘丝洞香惑女姬");
		} else if (exp <= 500000) {
			me->set("title", "盘丝洞幻丝魅娘");
		} else if (exp <= 1000000) {
			me->set("title", "盘丝洞凝香圣姬");
		} else if (exp <= 2000000) {
			me->set("title", "盘丝洞绝艳魔姬");
		} else {
			me->set("title", HIR"盘丝洞紫青妖皇"NOR);
		}

		tell_room(environment(me),
			HIM"" + ob->query("name") + "轻拢云鬓，纤指轻点：" + me->query("name") +
			"姿容无双，魅骨天成，已得盘丝正传。\n"
			"今以盘丝秘典之名，授予其「" + me->query("title") +
			"」之号，望你织情化劫，倾世摄魂，成就红尘妖皇之道！\n"NOR,
			({ me, me }));
	}
	return 1;
}
