...
void init()
{
	...
	me->create_identity("/d/wudang1/npc/yuanqiao", "/d/wudang1/shibapan");
...
string zm_apply()
{
	...
	ob->set_temp("wudang_zm_applied", 1);
...
int accept_fight(object ob)
{
	...
	if (ob->query_temp("wudang_zm_applied"))
	{
...
int check_result(object me, object ob)
{
	...
	if (((int)me->query("kee")*100/(1+my_max_kee)) <= 50 )
	{
		...
		if (me->query_temp("zhangmen/kill") && ob->query_temp("wudang_zm_applied"))
		{
			message_vision ("$N皱了皱眉道：此次比武我未能静心尽力，希望重新来过。\n", me);
			return 1;
		}

		if( ob->query_temp("wudang_zm_applied") != 1 )
			return 1;	
		...
		{
			message_vision("$N躬身对$n说道：恭请"+RANK_D->query_respect(ob)+"随我一同向师祖禀明出让掌门大弟子一事。\n", me, ob);
...
{
	CHANNEL_D->do_channel(who,"chat","本派太极天师"+ob->query("name")+"今日走马上任。恭请各位仙长多加捧场！");
	ob->delete_temp("wudang_zm_applied");
	me->set("new_player",1);
...
int rank_me()
{
	...
	exp=(int)me->query("combat_exp");
        if( me->query("title") == YEL"武当派"NOR WHT"太极天师"NOR
          ||me->query("title") == YEL"武当派"NOR WHT"太极天师"NOR)
	...
	{
                if( exp <= 20000 ){
			me->set("title", BLU"武当派伙房道童"NOR);
		} else if(exp <= 100000 ){
			me->set("title", HIB"武当派知客小童"NOR);
		} else if(exp < 500000 ){
			me->set("title", YEL"武当派复真观道人"NOR);
		} else if(exp <= 800000 ){
			me->set("title", HIY"武当派复真观掌教"NOR);
		} else if(exp <= 1500000 ){
			me->set("title", CYN"武当派太极宗师"NOR);
		} else if(exp <= 3000000 ){
			me->set("title", HIC"武当派百变真人"NOR);
		} else if(exp <= 5000000 ){
			me->set("title", WHT"武当派无根道尊"NOR);
		} else {
			me->set("title", HIW"武当派太极天师"NOR);
		}
...
HIR""+ob->query("name")+"对"+me->query("name")+"说：你可以做"+me->query("title")+"。\n"NOR, ({me, me}));
	...
	{
                if( exp <= 20000 ){
			me->set("title", BLU"武当派伙房女童"NOR);
		} else if(exp <= 100000 ){
			me->set("title", HIB"武当派沏茶女童"NOR);
		} else if(exp < 500000 ){
			me->set("title", YEL"武当派复真观道人"NOR);
		} else if(exp <= 800000 ){
			me->set("title", HIY"武当派复真观掌教"NOR);
		} else if(exp <= 1500000 ){
			me->set("title", MAG"武当派太极宗师"NOR);
		} else if(exp <= 3000000 ){
			me->set("title", HIM"武当派百变真人"NOR);
		} else if(exp <= 5000000 ){
			me->set("title", WHT"武当派无根道尊"NOR);
		} else {
			me->set("title", HIW"武当派太极天师"NOR);
