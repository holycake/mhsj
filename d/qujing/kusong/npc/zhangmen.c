...
void reset_me (object me)
{
	...
	me->set("gender", "男性" );
	me->set("title",HIR+me->query("family/family_name")+NOR+zname(me));
	me->set("short",RED+me->query("name")+NOR+"("+HIY+capitalize(me->query("id"))+NOR+")");
...
int rank_me()
{
	...
	if (me->query("title") == HIR"枯松涧积雷山碧波潭三山总洞主"NOR)
		return notify_fail("你已经是火云三山的总洞主，已无更高之位可授。\n");

	if (me->query("family/family_name") == "火云洞")
	{
		if (exp <= 20000) {
			me->set("title", "火云洞烧火小妖");
		} else if (exp <= 50000) {
			me->set("title", "火云洞守洞夜叉");
		} else if (exp <= 100000) {
			me->set("title", "火云洞喷焰使者");
		} else if (exp <= 200000) {
			me->set("title", "火云洞炼狱头目");
		} else if (exp <= 500000) {
			me->set("title", "火云洞副洞主");
		} else if (exp <= 1000000) {
			me->set("title", "火云洞火王使");
			...
			me->set("title", HIR"火云洞三山洞主"NOR);
		}

		tell_room(environment(me),
			HIR + ob->query("name") + "仰天狂笑，周身烈焰升腾，喝道：\n" +
			me->query("name") + "烈性如火，霸气如焚，堪掌我火云之权！\n"
			"今封其为「" + me->query("title") + "」，统摄洞妖，纵火焚天！\n"NOR,
			({ me, me }));
	}
	return 1;
}
