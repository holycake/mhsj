...
void create()
{
   ...
   set_skill("wuxing-quan", 999);
   set_skill("dodge", 999);
   set_skill("baguazhen", 999);
   set_skill("parry", 999);
   set_skill("sword", 999);
   set_skill("sanqing-jian", 999);
   set_skill("staff", 999);
   set_skill("fumo-zhang", 999);
   set_skill("hammer", 999);
   set_skill("kaishan-chui", 999);
   set_skill("blade", 999);
   set_skill("yange-blade", 999);
   set_skill("force", 999);   
   set_skill("zhenyuan-force", 999);
...
void attempt_apprentice(object ob)
{
  ...
  //}
   ...
   if( (string)ob->query("family/family_name")=="五庄观" )
   {
     ...
     {
        if( (int)ob->query("combat_exp") >= 3000000 && (int)ob->query_skill("taiyi", 1) >= 220 )
        {
          command("nod ");
