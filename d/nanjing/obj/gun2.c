...
void create()
{
        set_name(HIB"万胜佛狼机"NOR, ({ "folang ji", "folang", "chong", "gun" }));
        set_weight(2000);
        if (clonep())
        ...
        {
                set("unit", "把");
                set("long", "此铳由佛狼机铳改造而成。配有三根子炮，每次由母铳口更换。\n每次射击后虽需更换子炮，然较旧制为易。\n此铳威力甚大，平射可打二百步。\n你可以用它来射(shot)些什么。\n");
				set("value", 300);
                set("power", 5000);   // 攻击威力
...
int do_reload()
{
	...
	message_vision("$N开始给" + name() + "的子铳装填子药。。。\n", me);
...
int reloading(object me)
{
        ...
        {
        case 0:
                message_vision("$N正在给" + name() + "装填子铳。。。\n", me);
        }

        if (time() - (int)me->query_temp("last_shot") >= 3)
        {
