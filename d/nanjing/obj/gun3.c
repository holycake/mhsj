...
void create()
{
        set_name(HIY"三捷神机"NOR, ({ "sanjie ji", "sanjie", "chong", "gun" }));
        set_weight(2000);
        if (clonep())
        ...
        {
                set("unit", "把");
                set("long", "此铳柄有照门，上装三铳，各有照星，可以连续旋转射击三次。\n此铳平射可打一百三十步。\n你可以用它来射(shot)些什么。\n");
                set("value", 300);
                set("power", 4000);   // 攻击威力
                set("accuracy", 90);   // 准确度，百分百为上限
        }
        setup();
}
...
int reloading(object me)
{
		...
		for (count = 0, i = 0; i < sizeof(inv); i++) {
			if( inv[i]->is_bullet() )
			{
				if( (amount = inv[i]->query_amount() ) < 3 )
				{
					return notify_fail("弹药数量不足。\n");
				}
				ob = inv[i];
        ...
        if (time() - (int)me->query_temp("last_shot") >= 15)
        {
                me->set_temp("shot_times", 3);
