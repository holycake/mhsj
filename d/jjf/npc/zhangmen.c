...
void reset_me (object me)
{
	...
	me->set_name("大元帅", ({ "zhang men", "zhangmen" }) );
	me->set("gender", "男性" );
	me->set("title",zname(me));
	me->set("short",HIR+me->query("name")+NOR+"("+HIY+capitalize(me->query("id"))+NOR+")");
...
int convert_identity (object me, object ob)
{
	...
	ob->set("zhangmen/title_old",ob->query("title"));
        ob->set("zhangmen/title",HIR+ob->query("family/family_name")+zname(ob)+NOR);
...
string zname(object ob)
{
	if( (string)ob->query("gender") == "女性" ) 
		return RED"大唐天下兵马大元帅"NOR;
	else return RED"大唐天下兵马大元帅"NOR;
}
...
int rank_me()
{
	...
	exp=exp+dx;
        if( me->query("title") == RED"大唐天下兵马大元帅"NOR)
