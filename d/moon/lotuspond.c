...
int do_cai(string arg)
{
 ...
 if (me->query("kee") < (int)(me->query("max_kee")/3))
	return notify_fail("您先歇歇吧，别那么馋:)\n");
 	...
 	if (me->query("mark/moon_chimeng"))
 	{
 		me->delete("mark/moon_chimeng");
 	}
	...
	if ((int)me->query_skill("moondance", 1)< 200){
          me->improve_skill("moondance",random((int)me->query_skill("moondance",1)/10));
          me->improve_skill("dodge",random((int)me->query_skill("dodge",1)/10));
