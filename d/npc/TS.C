...
void create()
{
        set_name(HIW"唐僧"NOR,({"tang sanzang","sanzang","tang","heshang","bonze"}));
        set("gender","男性");
        set("age",40);
        set("per", 30);
        set("int", 30);
...
void die ()
{
  ...
  object where = environment (me);
     ...
     if( present("sun xingzhe",where) || present("zhu bajie",where)
        ...
        message("system",HIY"【"+HIW+"西天取经"+HIY+"】"+HIW+"如来佛祖"+HIY"：完蛋了，今儿真是完蛋了！唐三藏在"+query("target")+"被"+ob->query("name")+"吃掉了。\n"NOR,users());
     ob->add_maximum_force(50);
       ob->add_maximum_mana(50);
        ...
        tell_object(ob,"\n你得到了"+COMBAT_D->chinese_daoxing(10000)+"道行和"
                + "2500点潜能，10000点武学，50点内力和法力,而且又年轻了许多。\n" );
