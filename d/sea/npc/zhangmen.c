...
void reset_me (object me)
{
        ...
        me->set("gender", "男性" );
        me->set("title",HIC"东海龙神"NOR);

        me->set("short",CYN+me->query("name")+NOR+"("+HIY+capitalize(me->query("id"))+NOR+")");
...
int convert_identity (object me, object ob)
{
	...
	ob->set("zhangmen/title", HIC"东海龙神"NOR);
        ob->set("title",HIC"东海龙神"NOR);
        ob->save();
...
int check_result(object me, object ob)
{
        ...
        if (((int)me->query("kee")*100/(1+my_max_kee)) <= 50 )
        {
                ...
                if (me->query_temp("zhangmen/kill") && ob->query_temp("sea_zm_applied"))
                {
                        message_vision
...
int rank_me()
{
	...
	exp=(int)me->query("combat_exp");
        if( me->query("title") == HIC"东海龙神"NOR)
