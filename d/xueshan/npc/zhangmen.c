...
void reset_me (object me)
{
	...
	me->set("gender", "男性" );
	me->set("title",HIW+me->query("family/family_name")+NOR+zname(me));
	me->set("short",WHT+me->query("name")+NOR+"("+HIY+capitalize(me->query("id"))+NOR+")");
...
string zname(object ob)
{
	if( (string)ob->query("gender") == "女性" ) 
                return HIW"镇山凤凰明王"NOR;
        else return HIW"无法无天大邪圣"NOR;
}
...
int rank_me()
{
	...
	exp=exp+dx;
        if( me->query("title") == HIW"大雪山镇山凤凰明王"NOR
          ||me->query("title") == HIW"大雪山无法无天大邪圣"NOR)
