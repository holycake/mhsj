...
void reset_me (object me)
{
        ...
        me->set("gender", "男性" );
        me->set("title",HIW+BLK+"大力鬼王"NOR);
        me->set("short",HIW+BLK+me->query("name")+NOR+"("+HIY+capitalize(me->query("id"))+NOR+")");
...
int convert_identity (object me, object ob)
{
		...
		ob->set("zhangmen/title", "大力鬼王");
        	ob->set("title",HIW+BLK+"大力鬼王"NOR);
        } else {	
		ob->set("zhangmen/title", "地府女主");
        	ob->set("title",HIW+BLK+"地府女主"NOR);
        }	
        ob->save();
        ...
        if (ob->query("gender")=="男性") {
        	...
        	me->set_name(ob->query("name"), ({ "nu zhu", "nu zhu" }) );
	       	me->set("title",HIW+BLK+"地府女主"NOR);
	}       	

        me->set("short",HIW+BLK+me->query("name")+"("+HIY+capitalize(me->query("id"))+NOR+")");
        save_record(me, ob);
...
"你曾叛师欺祖，言无信行不轨，岂能出任掌门弟子一职！\n";
	 ...
	 ob->set_temp("death_zm_applied", 1);
        if( (string)ob->query("gender") == "男性" )
                return "好！师兄有意出任大力鬼王一职？我们就切磋一下吧！\n";
...
int rank_me()
{
	...
	exp=(int)me->query("combat_exp");
        if( me->query("title") == HIW+BLK+"地府女主"NOR
          ||me->quert("title") == HIW+BLK+"大力鬼王"NOR )
