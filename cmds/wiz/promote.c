...
sprintf("%s(%s) 正在 promote %s。 \n",me->name(1), geteuid(me), ob->name(1) ));
        if (new_status=="(apprentice)" || new_status=="(wizard)" || new_status=="(arch)"){
                mkdir("/u/"+ob->query("id"));
                if (file_size("/u/"+ob->query("id")+"/workroom.c")<0){
                        cp("/u/workroom.c","/u/"+ob->query("id")+"/workroom.c");
                        mkdir("/u/"+ob->query("id")+"/obj");
  tell_object(ob,HIC"\n\n欢迎你加盟魔幻世纪巫师组，你的巫师工作室已经自动生成!\n你以后可以用home指令回到自己的工作室！\n"NOR);
                }
        }
//        else obj->move(env);
  //      obj->write_prompt();			
