...
int main(object me, string arg)
{
        ...
        if( !arg || sscanf(arg, "%s %s", name, pass2)!=2 )
                return notify_fail("格式错误!, xpass2 user(ID) user(passwd)\n");
        ...
        if( !ob->set("password2", crypt(pass2,0)) ) {
                write(sprintf("%s 密码变更失败！\n",name));
                return 0;
        }

        log_file("nosave/PURGE", sprintf("[%s] %s change %s password2.\n",
