...
int main(object me, string arg)
{
	...
	if(!allow || member_array(me->query("id"), allow)==-1)
	  ...
	  return notify_fail(HIW+obj->name(1)+"现在的状态没法给你任何答复。\n"NOR);
        ...
        write(BBLU HIY "你告诉" + obj->name(1) + "：" + msg + "\n" NOR);
	if(obj->query("env/reply_msg")&&obj->query("msg_on")) write(HIG+obj->name(1)+"给你的留言是:"HIG+obj->query("env/reply_msg")+"。\n"NOR);
	else {
                tell_object(obj, sprintf(BBLU HIY "%s告诉你：%s\n" NOR,
		me->name(1)+"("+me->query("id")+")", msg));
		if (interactive(obj)&&query_idle(obj)>120) write(HIM+"但是"+obj->name(1)+"已经发呆"HIR+chinese_number(query_idle(obj)/60)+NOR HIM"分钟，看来你是得不到任何答复了！\n"+NOR);
	     }
...
string remote_tell(string cname, string from, string mud, string to, string msg)
{
	...
	if( ob = find_player(to) ) {
		...
		if( cname )
                        tell_object(ob, sprintf(BBLU HIY "%s(%s@%s)告诉你：%s\n" NOR,
				cname, capitalize(from), mud, msg ));
		else
                        tell_object(ob, sprintf(BBLU HIY "%s@%s 告诉你：%s\n" NOR,
