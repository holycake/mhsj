...
*/
	...
	if( !userp(who) && obj->value() ) { // give money to npc
	    ...
	    printf("你给%s%s。\n", who->name(), full_name);
	    message("vision", sprintf("%s给你%s。\n", me->name(), full_name), who);
	    message("vision", sprintf("%s给%s%s。\n", me->name(), who->name(), full_name), 
	            environment(me), ({me, who}));

	    if (!userp(who) && !userp(obj))
