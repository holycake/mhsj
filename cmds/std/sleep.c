...
int main(object me, string arg,object ppl)
{
...
//	if(me->query("age")<16)
//		return notify_fail(WHT"你还没有发育好，怎么办事？\n"NOR);
...
//         if(me->query("bonze/class")==obj->query("bonze/class"))
//                  return notify_fail("和尚和尼姑是不能破戒的．．．\n");
	if(me->query("bonze/class"))
		return notify_fail("别忘了，你可是出家人，不能破色戒，否则会遭雷劈的！\n");
	if((object)obj->query("bonze/class"))
		return notify_fail("人家是出家人是不能够破戒的，你还是省省吧!\n");

	if( !living(obj) )
		return notify_fail(obj->name() + "没法跟你睡了。\n"); 

//        if ( !me =ppl->query("couple/id") && !obj =ppl->query("id")  ) 
//   message("shout",HIM"〖市井传言〗听说"+me->query("name")+"("+me->query("id")+")和"+obj->query("name")+"("+obj->query("id")+")没有结婚而发生关系！\n"NOR,me,obj); 
//   message("channel:chat",HIM+"〖市井传言〗听说"+me->query("name")+"和"+obj->query("name")+"没有结婚而发生关系！\n"NOR,me); 
	...
	if(me->query("gender")=="男性"){
        ...
        if ( !me->query("couple/name"))
    CHANNEL_D->do_channel(this_object(),"rumor",HIM+"听说"HIW+me->query("name")+"("+me->query("id")+")"HIM"和"HIW+obj->query("name")+HIW+"("+obj->query("id")+")"HIM"没有结婚而发生关系！\n"NOR,me); 
               me->set("title",HIG"奸夫"NOR);
               obj->set("title",HIM"淫妇"NOR);
