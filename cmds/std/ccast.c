...
int main(object me, string arg)
{
	...
	if( sscanf(arg, "%s on %s", spl, trg)==2 ) {
		...
		else if(userp(target)&&target->query("age")<18&&!environment(target)->query("alternative_die")&&base_name(environment(target))!="/d/qujing/wuzhuang/anshi")
		return notify_fail("上天有好生之德，放过小孩子吧！\n");
	} else {
		spl = arg;
		target = 0;
	}
