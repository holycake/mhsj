...
"只见天上降下一朵七色云彩，$N纵身跃上，飞驰而去......\n\n"NOR, me);
        ...
        if( (string)me->query("family/family_name") == "火云洞") {
              message_vision(HBYEL+HIR" $N口捻个口诀，口中念动真言,突然“轰”地冒起一团红雾,$N随即隐入红雾中。 \n"NOR, me);
              }
...
//      if(arg=="stone") loc="/d/4world/entrance";^M
        ...
        else if(arg=="aolai") loc="/d/wuguan/school1";
        else if(arg=="wusheng") loc="/d/wusheng/room1";
        else if(arg=="moon") loc="/d/moon/ontop2";
        else if(arg=="lingtai") loc="/d/lingtai/gate";
        else if(arg=="putuo") loc="/d/nanhai/gate";
        else if(arg=="plxd") loc="/d/penglai/penglai";
        else if(arg=="cac") loc="/d/city/center";
        ...
        else if(arg=="wudang") loc="/d/wudang1/wdroad4";
              else if(arg=="shushan") loc="/d/shushan/road1";
              else if(arg=="penglai") {
                if(!(map=present("eastsea map", me)) && !wizardp(me)&& me->query("family/family_name") != "月宫") {
                        write("你在天上转了半天也不知该往那边飞。。。\n");
                        message_vision("\n$N失望地从云上跳了下来！\n", me);
                        return 1;
                        }
                if(me->query("family/family_name") != "月宫"&& !wizardp(me)&&(string)map->query("unit")!="张"){
                        write("你在天上转了半天也不知该往那边飞。。。\n");
                        message_vision("\n$N失望地从云上跳了下来！\n", me);
                        return 1;
                        }
//              write("你驾着祥云向东边海中飞去。。。\n\n");
                ...
                if(!(map=present("xueshan map", me)) && !wizardp(me)&& me->query("family/family_name") != "大雪山") {
                        write("你在天上转了半天也不知该往那边飞。。。\n");
                        message_vision("\n$N失望地从云上跳了下来！\n", me);
                        return 1;
                        }
