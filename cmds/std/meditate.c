...
//                remove_call_out("exec_int");
	...
	if( !stringp(me->query_skill_mapped("spells")) )
       ...
       if( this_player()->query("max_mana") >3000 && !wizardp(this_player()))
	{if( sen_cost < 100 ) return notify_fail("你最少要花 100 点「精神」才能练功。\n");
     }else{
	if( sen_cost < 20 ) return notify_fail("你最少要花 20 点「精神」才能练功。\n");
           }
