...
private mapping trusted_read = ([/*可以读*/
    "/":        ({ "(arch)","(master)","(admin)","(immortal)","(apprentice)","(wizard)",})
]);
...
private mapping exclude_read = ([/*不能读*/
    ...
    "u":        ({  "(arch)","(wizard)","(apprentice)","(immortal)",}),
    "include":  ({  "(immortal)",}),
    "d/obj/misc":      ({  "(arch)","(wizard)","(apprentice)","(immortal)",}),
...
//     if (wizhood(ob)=="(admin)") return 0;
     if (status=="(admin)" && wizhood(temp)!="(admin)" &&status=="(master)"&& wizhood(temp)!="(master)" ) return 0;
     if (status=="(arch)" && (wizhood(temp)!="(admin)"&& wizhood(temp)!="(master)" && wizhood(temp)!="(arch)")) return 0;
...
int valid_seteuid( object ob, string uid )
{
   ...
   if( wiz_status[uid] != "(admin)" && wiz_status[getuid(ob)] == "(admin)"&&wiz_status[uid] != "(master)" && wiz_status[getuid(ob)] == "(master)" )
   return 1;
   return 0;
}
