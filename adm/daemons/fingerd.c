...
//string *exclude =({});
string *exclude =({"gslxz","suannai","gslxzlhp","waiwai"});
...
string finger_all()
{
	...
	} else  	// wizard finger
	{
		...
		for(i=0; i<sizeof(ob); i++) {
			if( this_player() && !this_player()->visible(ob[i]) ) continue;
//			msg = sprintf("%s%-14s%-10s%-4s%-14s%-7s%s%-2s\n",
			msg = sprintf("%s%-14s%-10s%-4s%-14s%-7s%s ("+query_ip_port(ob[i])+")\n",
				msg, ob[i]->query("name"), ob[i]->query("id"),
...
string finger_user(string name)
{
	...
	// mon 8/27/97
	{
		...
		if(this_player()==body)
		  msg =sprintf("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
                                  "\t注册英文账号：\t%s\n\t注册中文账号：\t%s\n\t目前权限等级：\t%s\n"
			"\t电子邮件地址：\t%s\n\t个人主页地址：\t%s\n\t[ OICQ ]号码：\t%s\n\t上次退线地址：\t%s "+query_ip_port(body)+"("+BJTIME_CMD->chinese_time(1, ctime(ob->query("last_on")))+ ")\n\n%s\n",
			ob->query("id"),
			ob->query("name"),
...
//			ctime(ob->query("last_on")),
		...
		msg =  sprintf("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
"\t注册英文账号：\t%s\n\t注册中文账号：\t%s\n\t目前权限等级：\t%s\n\t电子邮件地址：\t%s\n\t个人主页地址：\t%s\n\t[ OICQ ]号码：\t%s\n\t上次退线地址：\t"+BJTIME_CMD->chinese_time(1, ctime(ob->query("last_on")))+ "\n\n%s\n\n━━━━━━━━━━━━━━━━━━━━━━━━━━━[魔幻世纪]\n"
,
			ob->query("id"),
			ob->query("name"),
member_array(ob->query("id"),exclude)==-1?SECURITY_D->get_status(name):"(master)",
...
//"━━━━━━━━━━━━━━━━━━━━━━━━━━[魔幻世纪]\n"
	...
	} else  	// wizard finger
	{
		msg =  sprintf("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
"\t注册英文账号：\t%s\n\t注册中文账号：\t%s\n\t目前权限等级：\t%s\n"
			"\t电子邮件地址：\t%s\n\t个人主页地址：\t%s\n\t[ OICQ ]号码：\t%s\n\t上次退线地址：\t%s "+ob->query("last_port")+"("+BJTIME_CMD->chinese_time(1, ctime(ob->query("last_on")))+ ")\n\n %s\n",
			ob->query("id"),
			ob->query("name"),
			SECURITY_D->get_status(name),
			ob->query("email"),
...
//			ctime(ob->query("last_on")),
 ...
 msg += sprintf("━━━━━━━━━━━━━━━━━━━━━━━━━━[魔幻世纪]\n");
		...
		if( objectp(body = find_player(name)) 
			...
			&& geteuid(body)==name ) {
			msg += sprintf(
				"%s（%s）目前正在从 %s ("+query_ip_port(body)+")连线，已发呆 %d 秒钟。\n", 
				body->name(1),
