			...
			if( !userp(me) || !userp(victim) ) {
				...
				&&	(random(my["sen"]*100/my["max_sen"] + my["int"]*my["cps"]) > 150) ) {
					...
					if( my["potential"] - my["learned_points"] < 100 )
						my["potential"] += 1;
					me->improve_skill(attack_skill, random( (int)me->query_skill("force", 1)*10));
				}
...
void announce(object ob, string event, string msg)
{
	switch(event) {
		case "dead":
			 message_vision(HIY"\n$N"HIY"口吐"HIR"鲜血"HIY"，眼皮狠狠的翻了一下，"HIR"死了～～\n\n"NOR, ob);
			// message_vision("\n$N死了。\n\n"NOR, ob);
			break;
		case "unconcious":
			message_vision(HIR"\n$N"HIR"神志迷糊，脚下一个不稳，倒在地上昏了过去。\n\n"NOR, ob);
			break;
		case "revive":
			message_vision("\n$N慢慢睁开眼睛，清醒了过来。\n\n", ob);
			break;
...
{
                killer->add("weiwang",-victim->query("combat_exp")*50/(1+killer->query("combat_exp")));
                if(victim->query("combat_exp")*50/(1+killer->query("combat_exp"))>0)
                        tell_object(killer,HIR"由于你残害同门，你的师门威望降低了！！\n"NOR);
        }
			...
			{			
	     		str=HIR+"杀人犯"+HIM+victim->name(1)+"在"+HIW+
	         	MISC_D->find_place(environment(killer))+HIM+"被"+killer->name(1)+HIM+"就地正法"+HIM+"，"+killer->name(1)+"得到了应有的奖励。";
	     		CHANNEL_D->do_channel(this_object(), "rumor",str);
			...
			{			
	         	...
	         	MISC_D->find_place(environment(killer))+HIM+"被"HIR"杀人犯"HIM+killer->name(1)+HIM+"杀死了"+HIM+"，"+killer->name(1)+"被玉皇大帝减少道行十五年作为处罚。";
	     		CHANNEL_D->do_channel(this_object(), "rumor",str);
	  ...
	  if(userp(killer)) {
		...
		if((int)victim->nk_reward(killer, reward)>0) {
		  ...
		  killer->add("daoxing",reward);
                  ...
                  tell_object(killer,"\n你得到了"+str+"道行。\n");
		  msg=sprintf("%s(%s)在%s杀死了%s(%s)，得到"+str+"道行。", 
		    killer->query("name"),killer->query("id"),
                             MISC_D->find_place(environment(this_player())),victim->query("name"),
//		    MISC_D->find_place(environment(killer)),victim->query("name"),
		    victim->query("id"));
...
void compare(object killer,object victim)
{
        ...
        int i,k;
             if(!(weiwang=victim->query("weiwang")))
                weiwang=victim->query("combat_exp")/200+1;
        if(weiwang<100)return;
