...
int perform(object me)
{
      ...
      skill1 = me->query_skill("unarmed",1);
       if( !(me->is_fighting() ))
              ...
              if( time()-(int)me->query_temp("taijiquan_end") < 3/2 )
...
if (objectp(weapon = me->query_temp("weapon")))
      ...
      if( random(skill) > dp )
      {
              if(userp(me))
//                      me->add("force",-120);
              message_vision(HIW"$N使出太极拳的绝招, $n顿时觉得眼前一花，身体一软，被$N打的鼻青眼肿！\n" NOR);
	me->set_temp("taiji-quan_per", 1);
	COMBAT_D->do_attack(me, target, me->query_temp("weapon"));

	me->delete_temp("taiji-quan_per");
...
//	me->receive_damage("kee", 50);
  ...
  me->delete("env/brief_message");
	...
	target->query_temp("kee",-100);
      ...
      me->reset_action();
          me->set_temp("taijiquan_end",time());
