...
int cast(object me, object target)
{
	...
	if((int)me->query_skill("dengxian-dafa", 1) < 40 )
		return notify_fail(HIC"你的登仙大法还不够纯熟。\n"NOR);

	if((int)me->query("mana") < 300 )
		return notify_fail("你的法力不足以变作神象伤敌！\n");

        msg = HIW"\n$N"NOR+HIW"趁"NOR"$n"HIW"不备，怒吼一声，转瞬间化作一只"HIY"黄牙老象"HIW"，"
                 +"伸出蛟龙般的鼻子向"NOR"$n"HIW"卷去！\n" NOR;
...
//printf("dp=%5d dg=%5d",dp,dg);
        ...
        if (dp<30)
          {           
          msg += HIW "不料"NOR"$n"NOR+HIW"早有准备，扭转身形轻舒猿臂一把抓住了$N"NOR+HIW"的鼻子！\n"
              +NOR"$N"NOR+HIW"疼得一声闷哼，身不由己的被$n"NOR+HIW"拖了过去！\n"NOR;
          ...
          {           
          msg += HIW "$n"NOR+HIW"连忙扭转身形运转神功将$N"NOR+HIW"的长鼻荡开。\n"NOR;
          message_vision(msg, me, target);
          me->start_busy(1+random(2));
          }
        else if (dg<50)
          {           
          msg += HIW "$n"NOR+HIW"纵身向后一跃，闪过了这一击。\n"NOR;
          message_vision(msg, me, target);
          me->start_busy(1+random(2));
          ...
          {           
          msg += HIW "$n"NOR+HIW"促不击防，被$N"HIW"的长鼻高高卷起，$N用力一甩，"
              +"$n"HIW"如断了线的风筝般飞了出去！\n"NOR;
          message_vision(msg, me, target);
