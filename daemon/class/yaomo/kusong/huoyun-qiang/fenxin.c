...
int perform(object me, object target)
{
	...
	int i, mypot,tapot,temp_dodge,temp_spells;
        ...
        ||      target->is_corpse()
        ||      target==me)
                return notify_fail(HIW"你要对谁施展这一招"HIR"「焚心似火」"HIW"？\n"NOR);

        if(!me->is_fighting())
                return notify_fail(HIR"「焚心似火」"HIW"只能在战斗中使用！\n"NOR);
...
random((int)me->query("max_force")/1+(int)me->query("force_factor")));
