...
int perform(object me, object target)
{
	string msg, str;
	object weapon;
	int myskill, wuyue,k, l;
	int ap, ap2, dp, dp2, pp;
	int youcps, mycor, mystr, youstr,fc;
	int damage_bonus, damage, wound;
	int delay;
	object ob;
	weapon = me->query_temp("weapon");
//            wuyue = (int)me->query_skill("wuyue-spear", 1);
	...
	if( random(youcps) < 15 || random(ap+dp+ap2+dp2)>(dp+dp2)) {
	    ...
	    if (target->query("eff_kee")<0 || (!living(target) && target->query("kee")<0))  
                        if(k<171) 
                      {str=target->name()+HIM"被"+me->name()+HIM"用火云洞绝招"HIW"「五岳封禅」"HIM"杀死了，听说尸体已经被捅成了马蜂窝！";
	    ...
	    if (target->query("eff_kee")<0 || (!living(target) && target->query("kee")<0))  
                        if(k<241) 
                      {str=target->name()+HIM"被"+me->name()+HIM"用火云洞绝招五岳封禅的第四式"HIW"「文章衡峰」"HIM"杀死了！";
	                 message("channel:rumor",HIM"〖无中生有〗某人："+str+"\n"NOR,users());
...
//     if( k<301 && l<181 ) 
//     if( (int)me->query_skill("spear")<201) 
	    if (target->query("eff_kee")<0 || (!living(target) && target->query("kee")<0))  
                if(random(300)<298 && random(me->query("kar"))<26 && k>240 ) 
                     {str=target->name()+HIM"被"+me->name()+HIM"用火云洞绝招五岳封禅第五式"HIW"「平冲中庸」"HIM"杀死了！";
	                 message("channel:rumor",HIM"〖无中生有〗某人："+str+"\n"NOR,users());
	               }
// 此招一出，必死
// ap2+pp>pp
...
// 同时满足，概率极其微小
	    ...
	    if (target->query("eff_kee")<0 || (!living(target) && target->query("kee")<0))  
                if(random(ap2+pp)>pp && k>300 && l>180 && random(300)>297 && random(me->query("kar"))>25) 
                      {str=target->name()+HIM"被"+me->name()+HIM"用火云洞绝招五岳封禅的终极必杀技"HIW"「五岳归来不看山」"HIM"杀死了！";
	                 message("channel:rumor",HIM"〖无中生有〗某人："+str+"\n"NOR,users());
	               }
