...
void copy_status(object me,object ob, string* dirs)
{
        ...
        me->set("combat_exp",ob->query("combat_exp")*9/10);
...
void die()
{
        ...
        bonus = owner_ob->query("combat_exp")*(1+random(3))/10000;
        owner_ob->add("combat_exp", bonus);
        owner_ob->add("potential",bonus/4);
        if(( owner_ob->query("combat_exp") > 500000 ) ||( owner_ob->query("faith")> 500 ) )
       {
         owner_ob->add("faith",0);
         owner_ob->set("Hell_Job/done",1);
        destruct(this_object());
         }
         else 
        {  
