...
string show_status()
{
        ...
        else //armor
        {
                ...
                status += "     <法宝收取力> " + status_bar(query("fabao/max_defense_shou"), query_temp("fabao/defense_shou"), YEL) + "\n"; 
        }
...
int ji(string target_id)
{
        ...
        if(fabao->query("series_no") != "1" ) return notify_fail(""+fabao->name()+"无法祭出！\n");
        ...
        message_vision(HIC"\n$N"HIC"抖足精神，大喝一声“看法宝！”，只见$n"HIC"「呼」地一下飞到半空，\n"NOR, attacker, fabao);
        message_vision(HIC"霎那间天色一变，风声大作！$n"HIC"带出一道低啸向$N"HIC"凌空击来！\n\n"NOR,  target, fabao);
...
//damage is determined by attacker and target's enchant and attacking fabao's power.
        ...
        a_dx = (int)attacker->query("combat_exp");
        ...
        d_dx = (int)target->query("combat_exp");
        ...
        if( d_fali*100/(a_fali+d_fali) > random(100) )
        {
                ...
                if( d_fabao_power > 0 )
                {
                        ...
                        if( d_fabao1 ) 
                                message_vision(HIW"只见霞光一闪，$N的$n"HIW"已跟"+fabao_name+HIW+"斗在一起！\n"NOR, target, d_fabao1);
                        if( d_fabao2 ) 
                                message_vision(HIW"只见霞光一闪，$N的$n"HIW"已跟"+fabao_name+HIW+"斗在一起！\n"NOR, target, d_fabao2);
                        ...
                        if( a_fabao_power > d_fabao_power )//possible to damage defense fabao...
                        {
                                if( random( a_fabao_power - d_fabao_power ) > 3 ) 
                                {
                                        if( d_fabao2 )
                                        {
                                                ...
                                                message_vision(HIC"结果血光大盛，$N"HIC"发出一声哀鸣退了下来。\n"NOR, d_fabao2);
                        ...
                        else if( d_fabao_power > a_fabao_power )//possible to damage attack fabao
                        {
                                if( random( d_fabao_power - a_fabao_power ) > 3 ) 
                                {
                                        ...
                                        message_vision(HIC"结果血光大盛，$n"HIC"发出一声哀鸣，居然被$N收了过去！\n"NOR, target, fabao);  
        ...
        message_vision(HIC"结果$N硬受$n"HIC"一记，却是毫发无伤！\n"NOR, target, fabao); 
